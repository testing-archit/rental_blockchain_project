...
export default function CreatePage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const txHash = (await create(form.rent, form.deposit, startTs, endTs, metadataCID)) as `0x${string}`;
    ...
    <div className="min-h-screen flex flex-col pb-24">
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10 md:py-14">
        ...
        <form onSubmit={submit} className="glass-panel p-6 md:p-8 space-y-8">
          ...
          <section>
            <h2 className="font-display text-xl font-bold mb-4 flex items-center gap-2">
              <span className="text-accent">03</span> Pricing (ETH)
            </h2>
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="relative">
                  ...
                  />
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-muted font-mono font-bold">ETH</span>
                </div>
              </div>
              <div>
